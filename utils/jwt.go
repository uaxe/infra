...
type GinJWTMiddleware struct {
	...
	// User can define own CustomLoginResponse func.
	CustomLoginResponse func(*gin.Context, int, string, time.Time, any)
...
func NewJWT(m *GinJWTMiddleware) (*GinJWTMiddleware, error) {
...
func (mw *GinJWTMiddleware) middlewareImpl(c *gin.Context) {
	...
	if int64(claims["exp"].(float64)) < mw.TimeFunc().Unix() {
		mw.unauthorized(c, 50014, mw.HTTPStatusMessageFunc(ErrExpiredToken, c))
		return
	}
...
func (mw *GinJWTMiddleware) LoginHandler(c *gin.Context) {
	...
	if mw.CustomLoginResponse != nil {
		mw.CustomLoginResponse(c, http.StatusOK, tokenString, expire, data)
		return
	}

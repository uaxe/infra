...
func (n *NAWSHandler) Offer(c *Connection) {
	if !n.client {
		_, _ = c.Conn.Write([]byte{IAC, DO, n.OptionCode()})
	}
}
...
func (n *NAWSHandler) HandleDo(c *Connection) {
	if n.client {
		_, _ = c.Conn.Write([]byte{IAC, WILL, n.OptionCode()})
		n.writeSize(c)
		go n.monitorTTYSize(c)
	} else {
		_, _ = c.Conn.Write([]byte{IAC, WONT, n.OptionCode()})
	}
}
...
func (n *NAWSHandler) writeSize(c *Connection) {
	_, _ = c.Conn.Write([]byte{IAC, SB, n.OptionCode()})
	payload := make([]byte, 4)
